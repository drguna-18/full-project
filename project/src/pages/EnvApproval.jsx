...
const EnvApproval = () => {
  ...
  useEffect(() => {
    async function fetchLicenses() {
      try {
        const response = await axios.get("http://localhost:5000/api/getEnvLicense");
  ...
  const approveLicense = async (id) => {
    try {
      const response = await axios.patch(`http://localhost:5000/api/approveEnvLicense/${id}`);
