...
const deleteLicense = async (id) => {
  ...
  return (
    <div className="card shadow p-4 mx-auto my-4" style={{ maxWidth: '800px' }}>
      ...
      {renderRow('Declaration Agreed', declarationAgreed === 'true' ? 'Yes' : 'No')}
      ...
      <div className="d-flex justify-content-end mt-4 gap-2">
        {!isApproved &&(<button className="btn btn-danger" onClick={()=>deleteLicense(id)}>Reject</button>)}
        {isApproved?(<button className="btn btn-success" disabled={true} onClick={()=>approveLicense(id)}>Approved</button>):(<button className="btn btn-success" onClick={()=>approveLicense(id)}>Approve</button>)}
      </div>
    </div>
  );
};
